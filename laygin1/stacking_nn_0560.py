...
#分词
train_first = pd.read_csv(r'./train_first.csv',engine='python',encoding='utf-8')  #初赛数据集
train = pd.read_csv(r'./train_second.csv')  #复赛数据集
test = pd.read_csv(r'./predict_second.csv')
full_all = pd.concat([train_first,train,test],ignore_index=True) 

jieba.load_userdict(r'./mydict.txt')
stop_words = ['的','了','个','这','那','是','在','就','去','有','还','也','与','和','及','或','或者',
              '都','就','和','到','我','上','下','左','右','你','它','他','她']
ign_p = re.compile(r"""[0-9a-zA-Z！，/？<br>n:-《》；。～：【】“■！|，”!'-.·-‘’@#￥%…、/\\&*\]\[（—+=）()\t\n\s\r]+""",re.S)

def split_words(words):
    tmp_words = re.sub(ign_p,'',words)
    cutted_words = ' '.join([i for i in jieba.cut(tmp_words) if i not in stop_words])
    return cutted_words
full_all['cutted_Dis'] = full_all.Discuss.apply(split_words)
full_all['Dis_length'] = full_all.cutted_Dis.apply(lambda s:len(s.split()))
full_all.to_csv(r'./full_cutted_20180316.csv',index=None)

dis_cnt = full_all.Discuss.value_counts().sort_values(ascending=False)
same_dis = dis_cnt[dis_cnt>1]

full_same_dis = pd.DataFrame()
for dis in same_dis.index:
    temp_df = full_all[full_all.Discuss==dis]
    full_same_dis = pd.concat([full_same_dis,temp_df],ignore_index=True)
    full_all.drop(temp_df.index,inplace=True)
a = []
for dis,gr in gbyDiscuss:
    if (True in gr['Score'].isnull().values) and (False not in gr['Score'].isnull().values):
        a.append(dis)
full_same_dis_test = pd.DataFrame()
for dis in a:
    temp_df = full_same_dis[full_same_dis.Discuss==dis]
    full_same_dis_test = pd.concat([full_same_dis_test,temp_df],ignore_index=True)
    full_same_dis.drop(temp_df.index,inplace=True)
full_same_dis_train = pd.DataFrame()
for dis,gr in full_same_dis.groupby('Discuss'):
    if (True not in gr['Score'].isnull().values) and (False in gr['Score'].isnull().values):
        temp_df = full_same_dis[full_same_dis.Discuss==dis]
        full_same_dis_train = pd.concat([full_same_dis_train,temp_df],ignore_index=True)
full = full_all
from sklearn.preprocessing import StandardScaler
full_unknown_dis = full[full.Dis_length==0].reset_index(drop=True)
full_unknown_dis['Score'] = full_unknown_dis.Score.fillna(full_unknown_dis.Score.mean())
full = full[full.Dis_length!=0].reset_index(drop=True)

full_same_dis_train.drop_duplicates(['Discuss','Score'],inplace=True)
full_same_dis_train = full_same_dis_train[full_same_dis_train.Dis_length!=0]
full = pd.concat([full,full_same_dis_train,full_same_dis_test],ignore_index=True)
full_same_dis.to_csv(r'./full_same_dis_filled_180316.csv',index=None)
full_unknown_dis.to_csv(r'./full_unknown_dis_filled_180316.csv',index=None)
full.to_csv(r'./full_tobe_classify_180316.csv',index=None)

full = pd.read_csv(r'./full_cutted_20180316.csv',engine='python')
full.loc[full.Dis_length==0,'cutted_Dis'] = '空白'
model_path = r'./full_dis_256.model'
def get_average_word2vec(tokens_list, vector, generate_missing=False, k=256):
    ...
    averaged = np.mean(vectorized, axis=0)
    return averaged

def get_word2vec_embeddings(vectors, clean_questions, col,generate_missing=False):
    embeddings = clean_questions[col].apply(lambda x: get_average_word2vec(x.split(), vectors, 
                                                                                generate_missing=generate_missing))
    return list(embeddings)
embeddings_arr = np.array(get_word2vec_embeddings(w2v_model, full,'cutted_Dis'))
np.save(r'./full_dis_256_4_11.npy',embeddings_arr)
train_index = full[full.Score.notnull()].index
test_index = full[full.Score.isnull()].index
train = full_arr[[i for i in train_index]]
y = full[full.Score.notnull()].Score
y = y.reset_index(drop=True)
test = full_arr[[i for i in test_index]]
vec_norm_df = pd.DataFrame({'norm':np.linalg.norm(full_arr,axis=1)})
mean_vec_score_1 = train[[i for i in y[y==1].index]].mean(axis=0)
mean_vec_score_2 = train[[i for i in y[y==2].index]].mean(axis=0)
mean_vec_score_3 = train[[i for i in y[y==3].index]].mean(axis=0)
mean_vec_score_4 = train[[i for i in y[y==4].index]].mean(axis=0)
mean_vec_score_5 = train[[i for i in y[y==5].index]].mean(axis=0)
from sklearn.preprocessing import scale
vec_norm_df['distance_1'] = scale(np.linalg.norm(full_arr-mean_vec_score_1,axis=1))
vec_norm_df['distance_2'] = scale(np.linalg.norm(full_arr-mean_vec_score_2,axis=1))
vec_norm_df['distance_3'] = scale(np.linalg.norm(full_arr-mean_vec_score_3,axis=1))
vec_norm_df['distance_4'] = scale(np.linalg.norm(full_arr-mean_vec_score_4,axis=1))
vec_norm_df['distance_5'] = scale(np.linalg.norm(full_arr-mean_vec_score_5,axis=1))
new_full = np.hstack([full_arr,vec_norm_df[['norm','distance_1','distance_2','distance_3','distance_4','distance_5']]])
np.save(r'./full_dis_256_4_11_add_6feats.npy',new_full)
...
import gensim
w2v_model = Word2Vec.load(r'./full_dis_256.model')
...
dataset_train_blends.to_csv(r'./dataset_train_blends_con_4_2.csv',index=None)
models_pred.to_csv(r'./models_pred_con_4_2.csv',index=None)
