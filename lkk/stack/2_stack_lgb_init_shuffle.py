...
def tfidf_process_ci(data,train_data,test_data):
    ...
    y = train_data['Score']  
    tf1 = TfidfVectorizer(ngram_range=(1,6),token_pattern='\w+',analyzer='word',max_features=1000000)
...
def tfidf_process_zi(data,train_data,test_data):
    ...
    print(data1.shape)  
    tf2 = HashingVectorizer(ngram_range=(1,6),lowercase=False)
    tf2.fit(data['cut_zi'])
...
#     'colsample_bylevel': 0.7,
    ...
    model = lgb.train(params,train_all,num_boost_round=3000,valid_sets=[train_all],verbose_eval=10)
...
if __name__ == "__main__":
    ...
    # train_data[['Id','lgb_ci_feat']].to_csv('../stack_cache/5_fold_lgb_ci_feat_init_shuffle.csv',index=False)
    # test_data[['Id','lgb_ci_prob']].to_csv('../stack_cache/5_fold_lgb_ci_prob_init_shuffle.csv',index=False)
    ...
    # train_data[['Id','lgb_zi_feat']].to_csv('../stack_cache/5_fold_lgb_zi_feat_init_shuffle.csv',index=False)
    # test_data[['Id','lgb_zi_prob']].to_csv('../stack_cache/5_fold_lgb_zi_prob_init_shuffle.csv',index=False)
    ...
    res['prob'] = result
    res[['Id','prob']].to_csv('lgb_ci_shuffle.csv',index=False)    
