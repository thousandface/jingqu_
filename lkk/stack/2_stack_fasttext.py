...
def fasttext_data(data,label):
    ...
    import codecs
    file=codecs.open("train.txt","w","utf-8")
...
def n_folds_train_fasttext(n_folds,data,X,test_hh):
    # skf = list(StratifiedKFold(y, n_folds,random_state=2018,shuffle=True))
    skf = KFold(X.shape[0], n_folds,random_state=2018,shuffle=True)
    y = X['Score']
    X = X['cutted_Dis'].values
    ...
    for i ,(train,test) in enumerate(skf):
            ...
            classifier = fasttext.supervised(train_file,'model.model',lr=0.01,epoch=50,loss='hs',dim=64,word_ngrams=1,label_prefix="__label__")
            result = classifier.predict_proba(X_test.tolist(),k=5)
            pred = [[int(sco) * proba for sco,proba in result_i] for result_i in result]
            y_sub = [sum(pred_i) for pred_i in pred]                      
            print(1 /(1 + mean_squared_error(y_test,y_sub)**0.5))
            print(xx_mse_s(y_test, y_sub))
            daset_blend_train[test,0]=y_sub
            test_result = classifier.predict_proba(test_hh['cutted_Dis'].tolist(),k=5)
            ...
            daset_blend_test_0[:,i] = fast_predi
    daset_blend_test[:, 0] = daset_blend_test_0.mean(1)

    return daset_blend_train,daset_blend_test

fasttext_ci_feat, fasttext_ci_prob = n_folds_train_fasttext(5,data,train_data,test_data)

train_data['fasttext_ci_feat'] = fasttext_ci_feat
test_data['fasttext_ci_prob'] = fasttext_ci_prob
train_data[['Id','fasttext_ci_feat']].to_csv('../stack_cache/5_fold_fasttext_ci_feat.csv',index=False)
test_data[['Id','fasttext_ci_prob']].to_csv('../stack_cache/5_fold_fasttext_ci_prob.csv',index=False)